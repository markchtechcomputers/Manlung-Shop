...
function initEmailSubscription() {
  ...
  subscribeBtn.addEventListener("click", () => {
    ...
    if (email.includes("@") && email.includes(".")) {
      let emails = [];
      try { emails = JSON.parse(localStorage.getItem("manlungNewsletterEmails") || "[]"); } catch(e) {}
      if (!emails.includes(email)) {
        emails.push(email);
        localStorage.setItem("manlungNewsletterEmails", JSON.stringify(emails));
      }
      window.cartFunctions.showToast("Subscribed! You'll get early access to new drops.");
      input.value = "";
    } else {
      window.cartFunctions.showToast("Enter a valid email address");
    }
  });
}

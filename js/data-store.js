...
(function () {
  ...
  function mergeTourShowsWithDefaults(parsed) {
    const defaults = window.DEFAULT_PRODUCT_DATA || {};
    const defaultShows = Array.isArray(defaults.tourShows) ? defaults.tourShows : [];
    const savedShows = Array.isArray(parsed?.tourShows) ? parsed.tourShows : [];

    if (!defaultShows.length) return parsed;
    const mergedShows = savedShows.map(show => {
      const defaultShow = defaultShows.find(d => d.id === show.id) || defaultShows[0] || {};
      const ticketTypes = Array.isArray(show.ticketTypes) ? show.ticketTypes.map((ticket, idx) => ({
        ...(defaultShow.ticketTypes?.[idx] || {}),
        ...ticket
      })) : (defaultShow.ticketTypes || []);
    ...
    parsed.tourShows = mergedShows.length ? mergedShows : defaultShows;
    return parsed;
  }
  ...
  function loadFromLocalStorage() {
    ...
    try {
      ...
      const merged = mergeTourShowsWithDefaults(parsed);
      merged.allProducts = [...(merged.digitalProducts || []), ...(merged.cdProducts || [])];
      window.productData = merged;
    } catch (e) {
  ...
  function rerenderEverything() {
    window.productData = mergeTourShowsWithDefaults(window.productData);
    ...
    window.renderFunctions?.renderTourShows();
